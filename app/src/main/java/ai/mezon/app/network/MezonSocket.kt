...
class MezonSocket @Inject constructor(
    ...
    /**
     * Build an Envelope via DSL, assign a CID, send it,
     * and suspend until the server responds with the same CID.
     */
    suspend fun send(block: EnvelopeKt.Dsl.() -> Unit): Envelope {
        val cid = "cid-${cidCounter.incrementAndGet()}"
        val env = envelope {
            this.cid = cid
            block()
        }
    ...
    /**
     * Send a fire-and-forget Envelope (no CID, no response expected).
     */
    ...
    private fun handleEnvelope(envelope: Envelope) {
        ...
        // Server-pushed event (no CID or unknown CID) — dispatch
    ...
    private fun scheduleReconnect() {
        ...
        reconnectJob = scope.launch {
            ...
            } catch (e: ai.mezon.app.session.SessionExpiredException) {
