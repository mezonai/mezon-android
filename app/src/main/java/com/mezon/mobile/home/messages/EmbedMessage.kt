...
class EmbedMessageRenderer(
    ...
    fun containsTouch(x: Float, y: Float): Boolean {
        ...
        for (g in embedInteractiveGeometries) {
            val r = g.rect
            if (x >= r.left && x <= r.right && y >= r.top && y <= r.bottom) return false
        }
        for (r in embedCardHitRects) {
    ...
    fun hitTestEmbedCardLink(x: Float, y: Float): String? {
        for (i in embedCardHitRects.indices) {
    ...
    fun setDataFromContent(content: String) {
        embedSourceList = if (content.contains("\"embed\"")) parseEmbedDataList(content) else emptyList()
        actionRows = if (content.contains("\"components\"")) parseEmbedActionRows(content) else emptyList()
    }

    /** Layout rects for inputs / selects / radios; valid after [draw]. */
    ...
    private fun embedFieldDrawOrder(fields: List<EmbedField>): List<Int> {
    ...
    private fun computeCardHeight(cardIdx: Int, card: LaidOutEmbedCard): Int {
        ...
        val animIxCount = IntArray(1)
        for (i in embedFieldDrawOrder(d.fields)) {
            val (nameLay, valLay) = card.fieldLayouts.getOrNull(i) ?: (null to null)
    ...
    /**
     * @return Y coordinate just below embed card and any component buttons.
     */
    ...
    private fun drawEmbedCard(
        ...
        embedCardHitRects.add(RectF(left, cardTop, left + cardW, cardTop + embedH))
            ...
            field.interactive?.let { iv ->
                when (iv) {
                    is EmbedFieldInteractive.Animation -> {
                        ...
                        val geom = when (iv) {
                            is EmbedFieldInteractive.Input -> EmbedInteractiveGeometry.InputField(
                                RectF(textLeft, y, textLeft + card.layoutLeftColumnW, y + blockH),
                                iv.componentId,
                                iv.input,
                            )
                            is EmbedFieldInteractive.Select -> EmbedInteractiveGeometry.SelectField(
                                RectF(textLeft, y, textLeft + card.layoutLeftColumnW, y + blockH),
                                iv.componentId,
                                iv.input,
                                field.name,
                            )
                            is EmbedFieldInteractive.Radio -> EmbedInteractiveGeometry.RadioField(
                                RectF(textLeft, y, textLeft + card.layoutLeftColumnW, y + blockH),
                                iv.componentId,
                                iv.input,
                            )
                        ...
                        embedInteractiveGeometries.add(geom)
                        y += blockH + GAP
                    }
                }
            }
    ...
    private fun drawButtons(canvas: Canvas, left: Float, top: Float): Float {
        ...
        for (b in laidOutButtons) {
            ...
            canvas.restore()
            buttonHits.add(EmbedButtonHit(RectF(l, tt, l + b.width, tt + b.height), b.buttonId, b.url, b.disabled))
        }
