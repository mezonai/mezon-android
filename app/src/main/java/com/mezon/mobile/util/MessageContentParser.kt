...
data class EmbedData(
    ...
    /** One or more main embed images (object, array of objects/URLs, or plain URL string). */
...
/** Same as [EMessageComponentType.BUTTON] in the web client. */
...
/**
 * Parses every object in the message JSON `embed` array (multi-embed / KOMU quiz-style messages).
 */
            ...
            for (i in 0 until fieldsArr.length()) {
                ...
                val interactive = f.optJSONObject("inputs")?.let { inputsObj ->
                    ...
                    val idResolved = idRaw.ifEmpty {
                        /**
                         * Some embed components (slots / sprite animation) omit `id`; the web client still renders them.
                         */
